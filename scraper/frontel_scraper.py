"""
Scraper de tarifas Frontel S.A. (Grupo SAESA, IX Región de La Araucanía).
...
_PDF_URL_BASE = "https://www.frontel.cl/informacion-de-tarifas/"
...
def parsear_pdf(path_pdf: Path) -> Optional[dict]:
    ...
    serv_pub   = buscar(["servicio público", "servicio publico"])
    ...
    if not all([cargo_fijo, serv_pub, energia]):
        logger.warning(f"Frontel: valores insuficientes. fijo={cargo_fijo}, energia={energia}")
        return None
    ...
    logger.info(f"Frontel: extracción exitosa → fijo={cargo_fijo}, energía={energia}")
    return resultado
...
def obtener_pdf() -> Optional[Path]:
    ...
    if url_pdf:
        return descargar_pdf(url_pdf, "frontel_tarifas_vigente.pdf")
    logger.error("No se encontró PDF de Frontel")
    return None
