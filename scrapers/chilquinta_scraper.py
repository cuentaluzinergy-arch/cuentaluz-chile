"""
scrapers/chilquinta_scraper.py — Chilquinta Energia S.A. (V Region)
...
class ChilquintaScraper(BaseScraper):
    DISTRIBUIDORA_ID = "chilquinta"
    PDF_URL_BASE     = "https://www.chilquinta.cl/clientes/tarifas/"
    ...
    def extraer_valores(self, filas: list):
        ...
        if not all([cargo_fijo, serv_pub, energia]):
            logger.warning(f"chilquinta: extraccion incompleta — fijo={cargo_fijo}, energia={energia}")
            return None
        ...
        logger.info(f"chilquinta: fijo={cargo_fijo}, energia={energia}")
        return resultado
